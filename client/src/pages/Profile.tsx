...
export const Profile = () => {
  ...
  const [selectedFilterPosts, setSelectedFilterPosts] = useState<string>('すべて');
  const [selectedFilterPurchases, setSelectedFilterPurchases] = useState<string>('すべて');
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        if (currentUser) {
          setLoading(true);
          const response = await axios.get(`${import.meta.env.VITE_API_URL}/users/${currentUser?.userId}`);
  ...
  const filteredPostedProducts = filterProducts(user.postedProducts, selectedFilterPosts);
  const filteredPurchasedProducts = filterProducts(user.purchasedProducts, selectedFilterPurchases);
    ...
    <div className='my-20 flex flex-col items-center justify-center'>
      ...
      <Tabs defaultValue="post" className="px-4 w-[400px]">
        ...
        <TabsContent value="post" className='mt-6'>
          <Select onValueChange={(value) => setSelectedFilterPosts(value)} value={selectedFilterPosts}>
          ...
          <ProductList
            products={filteredPostedProducts}
          />
        </TabsContent>
        <TabsContent value="buy" className='mt-6'>
          <Select onValueChange={(value) => setSelectedFilterPurchases(value)} value={selectedFilterPurchases}>
            <SelectTrigger className="w-[100px] h-[35px] bg-product-status-gray text-white mb-4">
              <SelectValue placeholder="すべて" className='font-normal' />
            </SelectTrigger>
            <SelectContent className='w-[80px] z-20 bg-white'>
          ...
          <ProductList
            products={filteredPurchasedProducts}
          />
        </TabsContent>
      </Tabs>
    </div>
